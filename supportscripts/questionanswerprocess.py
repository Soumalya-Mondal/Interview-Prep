...
def question_answer_process(database_file_path: str, system_prompt_file_path: str) -> dict[str, str]:
    ...
    try:
        database_connection.close()
        return {'status': 'SUCCESS', 'step': '6', 'file_name': 'Question-Answer-Process', 'message': 'All Answers Processed Successfully'}
    except Exception as error:
        return {'status': 'ERROR', 'step': '7', 'file_name': 'Question-Answer-Process', 'message': str(error)}
